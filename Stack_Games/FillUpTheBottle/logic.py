...
class FillUpTheBottle:
    ...
    def _initialize_game(self):
        total_bottles = self.colors + 2  # Adding two empty bottles (representing the "empty" bottles)
        bottles = [[] for _ in range(total_bottles)]
        balls = [color for color in range(1, self.colors + 1) for _ in range(4)]  # 4 balls of each color
